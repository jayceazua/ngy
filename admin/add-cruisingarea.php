...
$link_name = "Add New Cruising Area";
$ms = round($_GET["id"], 0);
$featured = 0;
$status_id = 1;

if ($ms > 0){
	$sql = "select * from tbl_cruisingarea where id = :ms";	
	$pdo_param = array(
		array(
			"id" => "ms",
			"value" => $ms,
	...
	if ($found > 0){
		...
		$link_name = "Modify Existing Cruising Area";
	}else{
		$ms = 0;
	}
}
...
include("head.php");
?>
<form method="post" action="cruisingarea-sub.php" id="cruisingarea-ff" name="ff" enctype="multipart/form-data">
...
$(document).ready(function(){
	$("#cruisingarea-ff").submit(function(){
