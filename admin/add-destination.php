...
$link_name = "Add New Destination";
$ms = round($_GET["id"], 0);
$featured = 0;
$status_id = 1;

if ($ms > 0){
	$sql = "select * from tbl_destination where id = :ms";	
	$pdo_param = array(
		array(
			"id" => "ms",
			"value" => $ms,
	...
	if ($found > 0){
		...
		$link_name = "Modify Existing Destination";
	}else{
		$ms = 0;
	}
}
...
include("head.php");
?>
<form method="post" action="destination-sub.php" id="destination-ff" name="ff" enctype="multipart/form-data">
    ...
    <table border="0" width="95%" cellspacing="0" cellpadding="5" class="htext">    
        ...
        <tr>
             ...
             <img src="../charterboat/destination/<?php echo $imagepath; ?>" border="0" style="max-width: 100%;" /><br />
             <a class="htext" href="javascript:delete_image('<?php echo $ms; ?>','imagepath','tbl_destination','id','charterboat/destination')">Delete Image</a>
             </td>
        </tr>
        <?php
        }else{
		?>
        <tr>
             <td width="" align="left" class="tdpadding1"><span class="fontcolor3">&nbsp;&nbsp;</span>Select Image [w: <?php echo $charterboatclass->destination_im_width; ?>px, h: <?php echo $charterboatclass->destination_im_height; ?>px]:<br />&nbsp;&nbsp;[Allowed file types: <?php echo $cm->allow_image_ext; ?>]</td>
             <td width="" align="left" class="tdpadding1"><input type="file" id="imagepath" name="imagepath" class="inputbox inputbox_size4" /></td>
        </tr>
        <?php
        }
		?>
...
$(document).ready(function(){
	$("#destination-ff").submit(function(){
		...
		if ($("#imagepath").length > 0) {
			if (!image_validation(document.ff.imagepath.value, 'n', '<?php echo $cm->allow_image_ext; ?>')){ return false; }
		}
